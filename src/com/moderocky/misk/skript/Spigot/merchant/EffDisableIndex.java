...
@Examples({
        "disable trade 0 of merchant {_merchy}"
})
@Since("0.1.5")
public class EffDisableIndex extends Effect {

    static {
        Skript.registerEffect(EffDisableIndex.class,
                "disable (trade|recipe) %number% of [merchant] %merchant%",
                "disable (trade|recipe) %number% of [(villager|[wandering ]trader)] %entity%"
        );
    }
    ...
    public String toString(@Nullable Event e, boolean debug) {
        if (entity) {
            return "disable trade " + number.toString(e, debug) + " of " + entityExpression.toString(e, debug);
        } else {
            return "disable trade " + number.toString(e, debug) + " of " + merchantExpression.toString(e, debug);
        }
    }
}
