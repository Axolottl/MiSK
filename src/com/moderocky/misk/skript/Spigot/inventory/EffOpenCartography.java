...
/**
 * Simple cartography inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Cartography Inventory")
@Description({
        "Open a cartography inventory."})
@Examples("open cartography table named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenCartography extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenCartography.class, "open cartography (table|gui) named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.CARTOGRAPHY, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open cartography table named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
