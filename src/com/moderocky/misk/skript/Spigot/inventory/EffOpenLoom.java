...
/**
 * Simple loom inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Loom Inventory")
@Description({
        "Open a loom inventory."})
@Examples("open loom named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenLoom extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenLoom.class, "open loom named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.LOOM, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open loom named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
