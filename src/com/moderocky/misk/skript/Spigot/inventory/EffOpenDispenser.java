...
/**
 * Simple dispenser inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Dispenser Inventory")
@Description({
        "Open a dispenser inventory."})
@Examples("open dispenser named \"Bob\" to all players")
@Since("0.2.0")
...
public class EffOpenDispenser extends Effect {
    ...
    static {
        Skript.registerEffect(EffOpenDispenser.class, "open dispenser named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.DISPENSER, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open dispenser named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
