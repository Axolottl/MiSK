...
/**
 * Simple grindstone inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Grindstone Inventory")
@Description({
        "Open a fake grindstone inventory."})
@Examples("open grindstone named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenGrindstone extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenGrindstone.class, "open [fake] grind[ ]stone named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.GRINDSTONE, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open grindstone named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
