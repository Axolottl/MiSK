...
/**
 * Simple smoker inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Smoker Inventory")
@Description({
        "Open a fake smoker inventory."})
@Examples("open smoker named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenSmoker extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenSmoker.class, "open [fake] smoker named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.SMOKER, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open smoker named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
