...
/**
 * Simple hopper inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Hopper Inventory")
@Description({
        "Open a hopper inventory."})
@Examples("open hopper named \"Bob\" to all players")
@Since("0.2.0")
...
public class EffOpenHopper extends Effect {
    ...
    static {
        Skript.registerEffect(EffOpenHopper.class, "open hopper named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.HOPPER, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open hopper named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
