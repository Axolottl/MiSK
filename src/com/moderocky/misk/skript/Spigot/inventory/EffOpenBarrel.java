...
/**
 * Simple barrel inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Barrel Inventory")
@Description({
        "Open a barrel inventory."})
@Examples("open barrel named \"Bob\" to all players")
@Since("0.2.0")
public class EffOpenBarrel extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenBarrel.class, "open barrel named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.BARREL, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open barrel named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
