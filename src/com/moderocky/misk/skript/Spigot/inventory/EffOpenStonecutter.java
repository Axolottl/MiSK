...
/**
 * Simple stonecutter inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Stonecutter Inventory")
@Description({
        "Open a stonecutter inventory."})
@Examples("open stonecutter named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenStonecutter extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenStonecutter.class, "open stone[ ]cutter named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.STONECUTTER, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open stonecutter named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
