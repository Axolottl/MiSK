...
/**
 * Simple fake ender chest inventory creation for 1.14+
 *
 * @author Moderocky
 */

@Name("Open Fake Ender Chest Inventory")
@Description({
        "Open a fake ender chest inventory."})
@Examples("open fake ender chest named \"Bob\" to all players")
@Since("0.2.0")

public class EffOpenEnderChest extends Effect {
    private Expression<String> stringExpression;
    private Expression<Player> playerExpression;

    static {
        Skript.registerEffect(EffOpenEnderChest.class, "open [fake] ender[ ]chest named %string% to %players%");
    }
    ...
    protected void execute(Event event) {
        ...
        for (Player player : players) {
            Inventory inventory = Bukkit.createInventory(player, InventoryType.ENDER_CHEST, string);
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "open fake ender chest named " + stringExpression.toString(event, debug) + " to " + playerExpression.toString(event, debug);
    }
}
